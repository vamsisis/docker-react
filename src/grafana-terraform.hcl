...
- Current Target Response Time: 
  {{ if .Values }}{{ range $refID, $value := .Values }}{{ $value }}{{ end }}{{ else }}(Value unavailable){{ end }}
- Severity: 
  {{ if .Values }}
    {{ if ge (index .Values "B") 1.5 }}Critical (Urgent)
    {{ else if ge (index .Values "B") 1.2 }}Critical
    {{ else if ge (index .Values "B") 0.8 }}Warning
    {{ else }}Info{{ end }}
  {{ else }}(Severity unavailable){{ end }}
{{ end -}}
{{ end -}}
